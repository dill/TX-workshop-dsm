...
shape <- readShapeSpatial("data/montrave-studyarea.shp", #proj4string = CRS(newproj),
                          repair=TRUE, force_ring=TRUE, verbose=TRUE)
# import shapefile for the points
stations <- readShapeSpatial("data/montrave-stations.shp", #proj4string = CRS(newproj),
                        repair=TRUE, force_ring=TRUE, verbose=TRUE)
...
row.names(est.table) <- c("s(x,y)", "design")
